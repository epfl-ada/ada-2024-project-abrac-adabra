...
def ligand_umap(unique_smiles_df, embeddings, dimensions = 3, plotting = True):
    ...
    umap_df = pd.DataFrame(X_transformed_UMAP, columns=[f'Ligand UMAP {i}' for i in range(1, dimensions+1)])
...
def cluster_ligand(merged_umap_df,n_clusters= 9, plotting = True):
    ...
    kmeans_labels_UMAP = KMeans(n_clusters=n_clusters, random_state=42).fit_predict(merged_umap_df.iloc[:, 1:4])
...
def prot_umap(filtered_best_ligand_ic50, merged_umap_df, dimensions = 3, n_clusters = 9,plotting = True):
    ...
    umap_prot_df = pd.DataFrame(X_prot_transformed_UMAP, columns=[f'Protein UMAP {i}' for i in range(1, dimensions+1)])
